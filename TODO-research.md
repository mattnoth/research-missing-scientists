...
- [ ] **AARO DoD establishment memo browser-download** — DoD memo establishing AARO (signed Deputy SecDef Kathleen Hicks, July 20, 2022; cited inline in `analysis/hypotheses.md` H9 Context as the operative establishing document alongside the 50 U.S.C. § 3373 statutory codification): `https://media.defense.gov/2022/Jul/20/2003039074/-1/-1/1/ESTABLISHMENT-OF-THE-ALL-DOMAIN-ANOMALY-RESOLUTION-OFFICE.PDF` — returns HTTP 403 to all automated agents (curl + WebFetch both blocked by media.defense.gov bot mitigation). Browser download → save to `appendices/primary-sources/aaro/ESTABLISHMENT-OF-THE-ALL-DOMAIN-ANOMALY-RESOLUTION-OFFICE.PDF`; add inline `[local archive]` citation alongside the URL once captured.
...
- [x] **Step D — apply Update blocks + factual corrections (mccasland / garcia / reza)** — Completed 2026-05-08 (commit `f8d4e46`, single push). 376 insertions / 21 deletions across the three case files. Inline factual corrections with revision-marker convention: mccasland PhD 1989 (was 1988); Pagosa Springs walkback (boots + green shirt located at second residence, not "missing with him"); reza Hardwick year 2014 (was 2015); plus seventeen other inline italic markers documenting refinements detailed in the appended Update blocks. Garcia naming question (parent prompt's "Eddison → Steven Abel" item) was a no-op for the case-file body — case file was already canonical "Steven Abel Garcia"; "Eddison" appears only in `logs/` documenting the prompt-error.

#### Phase 6 follow-ups
- [ ] **Eskridge Update block apply** — bundle and draft are pending from prior session; depth-pass coverage extended this session to mccasland/garcia/reza but not Eskridge. Schedule a separate apply session for eskridge.
- [ ] **Find-a-Grave memorial 284387277 Wayback retrieval (Reza)** — agent-level block hit Wayback this session. User-side curl is the operationally cleanest path; Phase 3 archival pipeline is the proper systemic fix.
- [ ] **House Oversight letter PDFs primary-source archives** — `appendices/primary-sources/mccasland/` and the equivalent garcia/reza directories should update the existing 403-framing to "WebFetch 403, curl 200" per the source-deepening cross-check confirming the four PDFs (FBI, DOE, DOW, NASA) are alive on curl.
- [ ] **Sentinel Network "The Ghost General" public-records claims (mccasland)** — DBE Consulting LLC SoS registration verification; Riverside Research $450M aggregate-contract framing via SAM.gov pull. Substantiation pass for next session.
- [ ] **Hardwick AFRL contract record + obituary verification** — DTIC contract record + Dignity Memorial Dayton OH obituary already cited in Update block; archival capture (Phase 3) would harden the T1/T2 chain.
- [ ] **NamUs alternate-route check (garcia)** — public-facing search interface does not return populated results without DIAMD authenticated access; flag for next-session route to confirm whether Garcia has a NamUs record corresponding to NM DPS M101688.
- [ ] **2:30 PM Twin Peaks Saddle anguish report (reza)** — currently T7 forum reconstruction; LASD records-request status check (public records only, no FOIA submission) would resolve. Original police-report language not located.
- [ ] **Daily Mail Chris Melore byline series (cross-case)** — direct dailymail.co.uk URL retrieval continues to fail at agent level for both garcia and mccasland. Phase 3 archival pipeline is the proper fix.
