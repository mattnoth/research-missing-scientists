...
Main-thread apply-step session executing the drafted `## Update — 2026-05-08` block from [logs/eskridge-research-bundle-2026-05-08.md](eskridge-research-bundle-2026-05-08.md) against `cases/eskridge.md`. Bundle was the Phase 5 / Eskridge research deliverable (separate session); this is the apply-only counterpart. Pre-flight: spot-check curl on 5 of 7 sampled bundle URLs returned 200 (Fox News, IBTimes UK Reid-disclosure, The Mirror, NTRS, Mountain Eagle); 2 Newsweek URLs returned 406 to curl regardless of UA (anti-bot fingerprinting per bundle's note that these were verified via WebFetch). Wikipedia URL verification surfaced 6 invented-page 404s the bundle's draft had populated speculatively — Franc Milburn (no wiki), Joshua LeBlanc (no wiki), Samuel Reid (entrepreneur — no wiki), Geometric Energy Corporation (no wiki), Jennifer Coffindaffer (no wiki), Eric W. Davis / Eric Davis (astrophysicist) (no wiki). DOGE-1 lives at `/wiki/DOGE-1` not `/wiki/DOGE-1_Mission_to_the_Moon`. All 6 invented-wiki links dropped; DOGE-1 path corrected. Wikipedia links retained (verified 200): Karoline Leavitt, James Comer, Eric Burlison, Kash Patel, Michael Shermer, Richard Hanania, Eugene Podkletnov, Ning Li (physicist), Harold E. Puthoff, Ross Coulthart, George Knapp (television journalist), Luis Elizondo, Bob Lazar, Demonstration_Rocket_for_Agile_Cislunar_Operations (DRACO), DOGE-1, Havana_syndrome, Zersetzung, Redstone_Arsenal, United_States_Space_Command, Missile_Defense_Agency, United_States_Army_Aviation_and_Missile_Command, WHNT-TV, WAFF. **Inline revision markers (3, minimal per convention)**: L22 Key Dates April 2026 entry (federal probe formal engagement — WH / House Oversight / FBI / DOE / DoW / NASA scope); L30 Richard Eskridge / NASA-Marshall paragraph (NTRS T1/T2 substantiation now documented across 8 papers including 2015 inductive pulsed plasma thruster work and PuFF Engine); L49 Milburn / Shellenberger paragraph (Milburn credentials more accurately retired British paratrooper / former British counter-intelligence officer / defense analyst per Hobart Substack 2025-01-06 + IBTimes UK 2026-05-07; Shellenberger venue specifically House Subcommittee on the Weaponization of the Federal Government per factually.co + Boykin Substack — both characterizations preserved per Append-don't-overwrite). **No marker on the 2020 interview paragraph** — DEW retiering applies only to Burlison-attributed framing (Anomaly #8 in Update block); Eskridge's 2020 self-reported claims remain Alleged. **Top-of-file revision marker** appended (single sentence, prior content untouched). **Update block** placed after Contradictions (line 106) and before Related Cases (line 108) with `---` rules above and below per visual standing convention. **Link discipline applied during apply pass**: subsequent in-block link repeats bared (Newsweek Huntsville Mystery, Hobart Substack, Boykin Substack, IBTimes UK federal-investigation / mystery-death / amy-eskridge-death-investigation, instituteforexoticscience.org, archive.org youtube-HOtsZSzpnhI, youtube.com fqKEgAVGxEU, youtube.com 9pmi9AMQHCg, factually.co both URLs, NTRS API URLs, yahoo.com death-genius); URLs already linked elsewhere in the existing case-file prose (above the Update block) bared on first occurrence within the Update block per first-occurrence-per-file rule (Newsweek who-is, Fox News 11th-scientist, NewsNation father-dead-scientist, IBTimes UK grieving-father, IBTimes UK mysterious-death-anti-gravity, The Mirror, Britannia Daily, Arab Tribune obituary, Michael Shellenberger Wikipedia). **Re-fetch flags** (11) added to TODO-research.md Phase 5 / new "Eskridge Update follow-ups" section: Daily Mail Eskridge URL discovery; Cybernews mirror; Wayback for instituteforexoticscience.org; congress.gov Shellenberger House Weaponization Subcommittee testimony; Alabama SoS direct query for HoloChron LLC; Madison County Coroner public-records status check (NOT submission); 2021 voice recording Whisper transcription (Phase 3); 2020 Rys/Sokol interview Whisper transcription (Phase 3); Reddit direct subreddit query; NTD News removed video lead; "Reid" disambiguation. **Phase 5 box ticked** in TODO-research.md (research bundle was prior session; apply step is this session). **Memory honored**: `feedback_conclusion_neutrality` (no verdict; both framings remain compatible explicitly), `feedback_xfiles_posture` (Mulder-side imbalance presented honestly with provenance dependency caveat at top of B1 + in Imbalance note — not softened), `feedback_source_trust` (IBTimes UK / Mirror / WION / Gazeta Express equal-tier with US T4), `feedback_link_verification` (verify-don't-trust caught 6 invented Wikipedia links + DOGE-1 path correction), `feedback_link_discipline` (within-Update-block + within-case-file dedupe applied per first-occurrence rule), `feedback_enrichment_priority` (Wikipedia + primary-source links applied to verified-existing pages only — no plausible-looking guesses retained), `feedback_response_format`. **Hard rules honored**: no new external research (apply-only); no contact; provenance dependency on Milburn/Reid preserved as standing caveat throughout Mulder side; tier+confidence on every new claim; imbalance presented not adjudicated; Daily Mail / Cybernews / web.archive.org / congress.gov / Alabama SoS / Coroner status all flagged not chased. **Subagent strategy**: none — apply step is mechanical execution of the bundle's pre-approved findings against the case file. **Concurrency**: single-thread apply, working tree clean at start. **Diff scale**: case file 132 → ~370 lines (one Update block of ~240 lines + 3 inline markers + 1 top-of-file marker append). **Apply notes for next sessions**: (1) Bundle's flagged re-fetch items now in TODO-research.md as checkboxes — pickup whenever Phase 3 archival pipeline + user-side curl access become available. (2) Phase 9a Huntsville hotbed analysis is queued via the Update block's B5 surface-scan; Eskridge / Joshua LeBlanc / Ning Li / James "Tony" Moffatt cluster surfaced and ready for dedicated session. (3) Phase 9b antigravity / exotic-propulsion domain map is queued via B3's domain-figure mapping; Podkletnov, Ning Li, Puthoff, Davis, Knapp, Coulthart, Shellenberger, Elizondo, Geometric Energy / DOGE-1, Institute for Exotic Science, HoloChron, EarthTech, TTSA, HAL5 all surfaced as domain-graph nodes ready for cross-case linking. (4) Phase 8 behavioral-patterns appendix gains the May 13 2022 Milburn-precursor + May 24 2022 Reid-precursor as the canonical Eskridge precursor-statement evidence — sweep across other 10 cases for parallel statements remains TODO.

## 2026-05-08 — Phase 6 apply (mccasland / garcia / reza Update blocks + factual corrections)

Main-thread apply-step session executing the three drafted `## Update — 2026-05-08` blocks from the Phase 6 depth-pass against `cases/mccasland.md`, `cases/garcia.md`, `cases/reza.md`. Pre-condition verified before starting: iTerm A's source-URL cleanup commit (`7fa8346`) on origin and visible in `git log`, parent prompt's hard-conflict warning on cases/mccasland.md L30 + cases/garcia.md L74 satisfied. Cross-file `grep -rn "Eddison"` confirmed: occurrences only in `logs/` (research records documenting that "Eddison" was a prompt-error, not a real alternate name); no occurrences in case files, dossier.md, analysis/, data/. No watcher escalation needed; case files were already canonical "Steven Abel Garcia" — the parent prompt's "Garcia rename" item was therefore a no-op for the body but the revision-marker conventions still applied to other corrections. **Inline factual corrections applied with revision-marker convention** `*(updated 2026-05-08 — [see GitHub for details](URL))*` per SESSION-PLAN standing rule: (mccasland) PhD year 1988 → 1989 inline at L25 + narrative L45 with full four-member dissertation committee detailed in Update block; "Battin supervised" framing kept inline + marker references full committee (Vander Velde, von Flotow, Battin, Gai); "white hair" kept inline + marker notes BCSO/ABQ Journal T3 says "gray"; Pagosa Springs walkback markers on three lines (light-green-shirt L86, items-believed-missing-with-him L90, Documented section items-missing summary L136) — green shirt and hiking boots were located by BCSO at McCasland's Pagosa Springs CO second residence; wallet, .38 revolver and holster, red backpack remain unaccounted for; DeLonge email block L171 marker notes seven-email six-attendee scheduled meeting via WikiLeaks emailids 2125, 2635, 5078, 9501, 14150, 51979, 3099 (Lockheed Skunk Works EVP Rob Weiss + retired Maj Gen Michael Carey + M. Fisher Clinton aide); Assessment paragraph L194 marker notes "an email" framing is narrower than the underlying record. (garcia) Daily Mail entry L16 now carries Chris Melore byline + original article-15722375 URL (`https://www.dailymail.co.uk/sciencetech/article-15722375/missing-nuclear-official-new-mexico-secrets.html`, 2026-04-11 14:37 EDT updated 16:45 EDT same day); CRITICAL SOURCE-TRACING NOTE L32 marker captures Fox 4 KC fourth documented KCNSC-outreach (Olivia Johnson, 2026-04-23) + 0xTars X post 2026-04-19 "two anonymous sources" pluralization claim; Inclusion Rationale L36 marker captures House Oversight letter naming via SFNM/Cormac Dodd attribution (T1 Congressional letter scope promotion, substantive employment claim still T6 / Alleged); APD attribution paragraph L42 marker captures first published unnamed-APD-spokesperson quotes ("no new developments"; "the FBI has reached out and is able to assist with the investigation"); Documented (T1) section gained new bullet for NM DPS "Other-Caution" status flag + 08/12/2025 record-header date anomaly + photo image ID 1891468 + current age 48; Speculated section L69 marker on Burlison "foreign operation" framing applied at cluster level rather than to Garcia specifically. (reza) Hardwick `(1950-2015)` → `(1950-2014)` inline at narrative L66 with marker referencing 5 T1/T2 sources (UNSW alumni profile, Dignity Memorial Dayton OH obituary, Wikipedia, Sentinel Briefing, Fortean Winds — all confirm January 5, 2014, age 63, stage-four metastatic breast cancer diagnosed 2012); Open Question 10 Hardwick year corrected at L197 + marker captures the AFRL tenure overlap with McCasland's command (May 2011 – Oct 2013) ~2 years and the 1982 LANL employment per UNSW; Three patent applications line L99 marker adds abandonment status (US-2003 abandoned 2004-06-03; US-2004 abandoned 2009-12-07; US-2010 abandoned 2012-12-01) + Russian RU2301276C2 grant (UTC 2007-06-20, invalid since 2012-09-18 for non-payment) + custody chain Boeing → UTC → PWR → Aerojet Rocketdyne → L3Harris → AE Industrial Partners (2026-01-09 sale, $845M majority stake; new entity reverts to "Rocketdyne" brand); Find-a-Grave bullet L169 marker notes Wayback retrieval NEGATIVE this session (agent-level block; flag for user-side curl); Verification "Co-inventor" paragraph L220 marker adds Fortune 2026-04-21 commercial-aerospace tie (New Glenn / Starship inheritance pathway, both SpaceX and Blue Origin declined to comment) + 2016-09-06 GlobeNewswire press release on first Mondaloy 200™ public rocket-engine hot-fire test at Edwards AFB Test Stand 2A. **Top-of-file revision markers** updated on all three case files. **Update blocks appended at end of file** = full content of the three drafts excluding Apply notes + Imbalance + Anomalies appendix sections (those are reference for the apply session and not for case-file body, per draft instructions). **Out-of-scope dirty files at session start**: `analysis/hypotheses.md` (H9 AARO citation upgrade) and `cases/loureiro.md` (FBI Boston/USAO closure Update block) — both intermediate work product of the smalls session; smalls session committed those as `a4c483d` between this session's initial fetch and commit; my push fast-forwarded cleanly on top of `a4c483d`. **My commit `f8d4e46` contains only `cases/{mccasland,garcia,reza}.md`** (376 insertions, 21 deletions). **Apply notes for next sessions:** (1) Eskridge Update block apply still queued (Phase 6 depth-pass bundle + draft already exist; this session covered only mccasland/garcia/reza). (2) Phase 3 archival pipeline (Playwright + headed browser) remains the proper fix for the dailymail.co.uk / lamag.com / oversight.house.gov / web.archive.org agent-level blocks. (3) User-side curl tasks flagged: Find-a-Grave memorial 284387277 Wayback retrieval (Reza); House Oversight letter PDFs already accessible per smalls / source-deepening cross-check. (4) Open Questions added across the three case files (mccasland Q11/Q12; garcia Q8/Q9/Q10) — covered by Update-block content. **Subagent strategy:** none — apply step is mechanical execution of pre-approved drafts. **Concurrency:** safe — no overlap with smalls session (smalls touched analysis/hypotheses.md + cases/loureiro.md + logs/known-unknowns.md + TODO-research.md, this session touched cases/{mccasland,garcia,reza}.md). **Memory honored:** `feedback_xfiles_posture`, `feedback_conclusion_neutrality`, `feedback_voice_register`, `feedback_inline_source_urls`, `feedback_link_verification`, `feedback_link_discipline`, `feedback_enrichment_priority`, `feedback_source_trust`, `feedback_response_format`. **Verification:** push successful; `git log --oneline -3` shows `f8d4e46` on top of smalls `a4c483d` on top of source-URL cleanup commits; branch in sync with origin.

## 2026-05-08 — Smalls bundle (AARO H9 backfill + Loureiro FBI-closure + Wikipedia-Birmingham known-unknown)

Three small sub-tasks bundled into one fresh session because each was too small to warrant its own. **Sub-task 1 (AARO H9 Phase 1 backfill):** WebFetched `aaro.mil` (403), `defense.gov/Spotlights/AARO/` (403), `congress.gov/bill/117th-congress/senate-bill/1605` (403), `govinfo.gov/app/details/PLAW-117publ81` (200 but content-empty). WebSearch surfaced two T1 candidates: the DoD establishment memo PDF on `media.defense.gov` (`/2022/Jul/20/2003039074/-1/-1/1/ESTABLISHMENT-OF-THE-ALL-DOMAIN-ANOMALY-RESOLUTION-OFFICE.PDF`, signed Deputy SecDef Kathleen Hicks, July 20, 2022) and the codified statute at `uscode.house.gov/view.xhtml?req=(title:50+section:3373+edition:prelim)`. WebFetch on the DoD PDF: 403; curl with browser UA: 403 (529-byte HTML challenge response). WebFetch on uscode.house.gov: 200 + content-confirmed (codifies NDAA 2022 §1683 establishing AARO). Curl HEAD on uscode.house.gov: HTTP/2 200. Decision: cite uscode.house.gov inline as primary T1 (statutory codification is the highest-tier accessible primary); flag DoD memo PDF in TODO-research.md as a browser-download item (AARO/local-archive Phase 3 work). Wikipedia AARO article cross-checked the canonical-source ranking (DoD memo + statute) — independent confirmation that uscode.house.gov is the right primary anchor. Inline cite restructured per `feedback_link_discipline` to "All-domain Anomaly Resolution Office (AARO)" first-use form. **Sub-task 2 (Loureiro FBI-closure 2026-04-29 Update block):** grep `logs/` for "loureiro\|fbi\|closure" surfaced [logs/news-refresh-2026-05-08.md L37-39](news-refresh-2026-05-08.md) (C's news-refresh) confirming the 2026-04-29 announcement. Source URL not captured in logs; WebSearched `"FBI U.S. Attorney Massachusetts April 29 2026 Cláudio Valente Loureiro Brown shooting 'no nexus to terrorism'"` → surfaced `fbi.gov/contact-us/field-offices/boston/news/fbi-and-us-attorneys-office-for-the-district-of-massachusetts-release-findings-on-brown-university-and-brookline-shootings` as the canonical T1 + multiple T3/T4 mirrors (Brown Daily Herald, Boston Globe, WPRI, Rhode Island Current, NewsNation, Boston 25, NBC News, KCBY, CBS Boston). WebFetch FBI URL: 403; curl with browser UA: 403 (Cloudflare cf-mitigated challenge — confirmed bot-block, not URL-rot). WebFetch Brown Daily Herald URL: 200 + full content extracted (publish 2026-04-29 6:59 PM EDT, byline Emily Feil; verbatim quotes "worked alone," "no nexus to terrorism," "symbolic in nature," "personal failures and injustices he perceived were inflicted by others over time," "Only Neves Valente knew the real reason why he committed these heinous acts," investigation scale numbers — 112+ evidence items / 490+ leads / 11,000+ surveillance files / 815 videos / 1,327 audio files / 260+ interviews — mental-health framing "failure to thrive" / "long-standing suicidality" with cautionary "stressors alone cannot fully explain the attacks"). Update block drafted with both URLs (FBI Boston primary + Brown Daily Herald T3 mirror per `feedback_inline_source_urls` 403-footnote convention). Top-of-file revision marker added per SESSION-PLAN standing rule. **Sub-task 3 (Wikipedia Eskridge Birmingham-AL error annotation):** WebFetched `en.wikipedia.org/wiki/Amy_Eskridge` → no dedicated Eskridge article; redirected to "Missing scientists conspiracy theory" article. Captured stable revision URL: `https://en.wikipedia.org/w/index.php?title=Missing_scientists_conspiracy_theory&oldid=1353220678`. Verbatim wording: "Eskridge's death was investigated by Birmingham, Alabama police and ruled a suicide." grep `cases/eskridge.md` for "birmingham\|huntsville\|madison\|police": case file correctly references Huntsville PD + Madison County Coroner (lines 41, 125); does NOT propagate the Birmingham error. New `## Recursive-Citation Hazards` section appended to `logs/known-unknowns.md` with explicit scope clarification (the *jurisdiction* is wrong; the *suicide* characterization itself remains an open question elsewhere in the same file — cross-link added). Per CLAUDE.md no-contact-and-public-only-sources rule, Wikipedia is not edited by this dossier. **Apply pass:** four edits (analysis/hypotheses.md revision marker + H9 Context line; cases/loureiro.md revision marker + Update block; logs/known-unknowns.md new section; TODO-research.md AARO PDF browser-download entry). git diff --stat: 4 files, 24 insertions, 2 deletions. **Concurrency note:** working tree carried iTerm A's in-progress edits to cases/garcia.md (114 lines), cases/mccasland.md (120 lines), cases/reza.md (163 lines) at apply time — these are A's depth-pass apply work, NOT this session's. Staged only the four files this session touched; A's edits left unstaged for A's own commit. Push race handled by fast-forward pull before push. **Three sub-tasks completed cleanly; no contact made; no Wikipedia edited; no `cases/{eskridge,mccasland,garcia,grillmair}.md` touched.**
